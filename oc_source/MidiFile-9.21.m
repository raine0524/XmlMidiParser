...
- (void)sortEvents:(NSMutableArray*)midiEvents
{
    [midiEvents sortWithOptions:NSSortConcurrent usingComparator:^NSComparisonResult(Event *obj1, Event *obj2) {
        if (obj1.tick==obj2.tick) {
            ...
            if (evt1!=evt2) {
                if (evt1==0x80 || (evt1==0x90&&obj1.vv==0)) { //note off
...
- (void)dumpMidi:(NSArray*)midiEvents {
    ...
    for (int i=0; i<midiEvents.count; i++) {
        ...
        int oct=e.nn/12-1;
        int step=e.nn%12;
        [str appendFormat:@"%d: %d: %x %x %d (%d%s)\n",i, e.tick, e.evt,e.nn,e.vv,oct,abc[step]];
    }
    NSLog(@"%@",str);
}

- (NSArray*)arrayWithNoZeroLengthEvent:(NSArray*)midiEvents {
    //检查是否有重叠的event：同一个音，前面还没有结束，后面就又开始了。
//    NSMutableArray *zeroLengthEvents=[NSMutableArray new];
    for (int i=0; i<midiEvents.count; i++) {
...
//                            [zeroLengthEvents addObject:event];
//                            [zeroLengthEvents addObject:nextEvent];
                            nextEvent.tick+=1;
                            break;
                        }
                    }
...
//    if (zeroLengthEvents.count>0) {
//        NSMutableArray *newArray=[NSMutableArray arrayWithArray:midiEvents];
//        [newArray removeObjectsInArray:zeroLengthEvents];
//        return newArray;
//    }
    return midiEvents;
}
...
- (NSArray*)mergedMidiEvents {
    if (_mergedMidiEvents==nil) {
        ...
        }else if (self.tracks.count>self.maxTracks) {
        ...
        for (int i=0;i<self.tracks.count;i++) {
            ITrack *track = self.tracks[i];
//            [self dumpMidi:track.events];
//            NSArray *events=[self arrayWithNoZeroLengthEvent:track.events];
//            [self dumpMidi:events];
