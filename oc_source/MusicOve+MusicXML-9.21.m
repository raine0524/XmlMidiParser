...
- (NoteType)noteType:(NSString*)type{
            ...
            if ([name isEqualToString:@"duration"]) {
                ...
                */
                ...
                while (tuplet_elem) {
                    ...
                    TBXMLElement *tuplet_normal_elem=[TBXML childElementNamed:@"tuplet-normal" parentElement:tuplet_elem];
                    if (tuplet_normal_elem) {
                        TBXMLElement *tuplet_type_elem=[TBXML childElementNamed:@"tuplet-type" parentElement:tuplet_normal_elem];
                        NSString *tuplet_type_str=[TBXML textForElement:tuplet_type_elem];
                        tuplet_type=[self noteType:tuplet_type_str];
                        
                        if (note_arts==nil) {
                            note_arts=[[NSMutableArray alloc]init];
                        }
                        NoteArticulation *art=[NoteArticulation new];
                        [note_arts addObject:art];
                        if (note_type==Note_Eight) {
                        ...
                        art.tremolo_beem_mode=YES;
                        
                        note_type=tuplet_type;
                    }
                    
                    if (tuplet_type==Note_None && [type isEqualToString:@"stop"]) {
                        
                        //analyze never read
//                        BOOL foundStartTuplet=NO;
                        for (int prevN=(int)measure.notes.count-1; prevN>=0; prevN--) {
                            OveNote *prevNote=measure.notes[prevN];
                            if (prevNote.xml_tuplets) {
                                for (NSDictionary *dict in prevNote.xml_tuplets) {
                                    NSNumber *prev_number=dict[@"number"];
                                    NSString *prev_type=dict[@"type"];
                                    if (prev_number.intValue == number.intValue && [prev_type isEqualToString:@"start"]) {
                                        //analyze never read
//                                        foundStartTuplet=YES;
                                        NoteType start_tuplet_type=[dict[@"tuplet-type"] intValue];
                                        if (start_tuplet_type!=Note_None) {
...
//                            }
                            ...
                            if (art.art_type==Articulation_Tremolo_Eighth) {
                                ...
                                NSString *type=[TBXML valueOfAttributeNamed:@"type" forElement:ornaments_child_elem];
                                if ([type isEqualToString:@"single"]) {
                                    art.tremolo_stop_note_count=0;
                                }else if ([type isEqualToString:@"start"]) {
                                    art.tremolo_stop_note_count=1;
                                }else{
                                    [note_arts removeLastObject];
                                }
                            }
//                            if (has_wavy_line) {
...
//                    if (trill_art)
                ...
                if (arpeggiate_elem) {
                    ...
                    NSString *num=[TBXML valueOfAttributeNamed:@"voice" forElement:arpeggiate_elem];
                    if (num) {
                        art.arpeggiate_over_voice=[num intValue];
                    }
                    num=[TBXML valueOfAttributeNamed:@"staff" forElement:arpeggiate_elem];
                    if (num) {
                        art.arpeggiate_over_staff=[num intValue];
                    }
...
//                            static OveWedge *opened_wedge[2]={nil,nil};
                            ...
                            if (![wedge_type isEqualToString:@"stop"]) { //start
                                ...
                                if (number==0) {
                                    self.opened_wedge1=wedge;
                                }else{
                                    self.opened_wedge2=wedge;
                                }
//                                opened_wedge[number]=wedge;
                                ...
                                OveWedge *wedge;
                                if (number==0) {
                                    wedge=self.opened_wedge1;
                                }else{
                                    wedge=self.opened_wedge2;
                                }
                                wedge.offset.stop_measure=measure.number-wedge.offset.stop_measure;
                                wedge.xml_stop_note=(int)measure.notes.count;//the wedge end with next note
                                wedge.xml_staff=staff+start_staff;
                                if (wedge.offset.stop_measure==0 && wedge.xml_stop_note==wedge.xml_start_note) {
                                    if (wedge.xml_start_note>0) {
            ...
            }else if ([name isEqualToString:@"barline"])
            {
                ...
                if (ending_elem) {
                    ...
                    if ([barline_ending_type isEqualToString:@"start"]) {
                        ...
                        NSString *play=[TBXML valueOfAttributeNamed:@"play" forElement:ending_elem]; //yes or no, default is yes
                        if (play && [play isEqualToString:@"no"]) {
                            ending.ending_play=NO;
//                            ending.numeric_measure_count=0;
                        }else{
                            ending.ending_play=YES;
                        }
...
//    NSString *xml=[[NSString alloc] initWithData:pageData encoding:NSUTF8StringEncoding];
//    if (xml==nil) {
//        xml=[[NSString alloc] initWithData:pageData encoding:NSUTF16StringEncoding];
//        if (xml) {
//            pageData=[xml dataUsingEncoding:NSUTF8StringEncoding];
//        }
//    }
//    NSString *xml=[[NSString alloc] initWithData:pageData encoding:NSUTF16StringEncoding];
//    if (xml) {
//        pageData=[xml dataUsingEncoding:NSUTF8StringEncoding];
//    }
    ...
    do {
        ...
        if ([name isEqualToString:@"work"]) {
            ...
            self.xml_page_height=page_height;
...
//    int pages=0;
    for (int pageNumber=0;pageNumber<self.pages.count;pageNumber++) {
        OvePage *page = self.pages[pageNumber];
...
#define VELOCITY_LOW    70
    ...
    if (self.lines.count>1) {
        ...
        for (LineStaff *ls in lastLine.staves) {
            NSLog(@"%d",ls.y_offset);
        }
        self.page_height=lastLine.y_offset+LINE_height*5+lastStaff.y_offset+200;
        if (lastLine.staves.count==4) {
            self.page_height=lastLine.y_offset+LINE_height*4*lastLine.staves.count+lastLine.xml_staff_distance*2+lastLine.xml_system_distance+200;
        }
    }
#endif
    [self processLyrics];
...
//    int slur_line=(above)?note.line+1:note.line-1;
    ...
    if (note.note_elems.count>1) {
        NoteElem *firstElem,*lastElem;
        if (note.sorted_note_elems) {
            firstElem=note.sorted_note_elems.lastObject;
            lastElem=note.sorted_note_elems.firstObject;
        }else{
            firstElem=note.note_elems.lastObject;
            lastElem=note.note_elems.firstObject;
        }
        if (firstElem.line>=lastElem.line) {
            if (above) {
...
- (int)tappedNoteElems:(signed char*)values note:(OveNote*)note nextNote:(OveNote*)nextNote art:(NoteArticulation*)art below_note:(int)below_note upper_note:(int)upper_note {
    int values_count=0;
    if (art.art_type>=Articulation_Major_Trill && art.art_type<=Articulation_Turn) {
        NoteElem *lastElem=note.note_elems.lastObject;
        ...
        if (art.tremolo_stop_note_count>0 && nextNote) {
            values_count-=1;
//            totalOf64th/=4;
            NoteElem *lastElem1=note.note_elems.lastObject;
            NoteElem *lastElem2=nextNote.note_elems.lastObject;
            if (note.note_elems.count==1) {
...
- (void)setEventTrack:(Event*)event note:(OveNote*)note elem:(NoteElem*)elem  videoMidi:(BOOL)videoMidi midiFile:(MidiFile*)midiFile index:(int)index{
    if (videoMidi) { //video midi use normal midi's track info
        event.track=(elem.rightHand)?0:1;
        
        for (int i=index+1,len=(int)midiFile.mergedMidiEvents.count; i<len; i++) {
            Event *next=midiFile.mergedMidiEvents[i];
            unsigned char evt=next.evt&0xf0;
            if (evt==kMIDIMessage_NoteOff || (evt==kMIDIMessage_NoteOn && next.vv==0)) {
                if (next.nn==event.nn) {
...
- (void)setEventUserdata:(Event*)event from:(int)tt midiEvents:(NSArray*)midiEvents channel:(int)channel mm:(int)mm nn:(int)nn i_notes:(int)i_notes ee:(int)ee elem:(NoteElem*)elem measure:(OveMeasure*)measure note:(OveNote*)note meas_start_tick:(int)meas_start_tick trill:(BOOL)trill videoMidi:(BOOL)videoMidi midiFile:(MidiFile*)midiFile {
    ...
    if (videoMidi) { //video midi use normal midi's track info
        event.track=(elem.rightHand)?0:1;
        for (int i=tt+1,len=(int)midiFile.mergedMidiEvents.count; i<len; i++) {
            Event *next=midiFile.mergedMidiEvents[i];
            unsigned char evt=next.evt&0xf0;
            if (evt==kMIDIMessage_NoteOff || (evt==kMIDIMessage_NoteOn && next.vv==0)) {
                if (next.nn==event.nn) {
...
#endif
    midiFile.maxTracks=(int)self.trackes.count;
...
#define MAX_NOTE_ELEMS  512
                    ...
                    if (art) {
                        ...
                        OveNote *nextNote=nil;
                        for (int n=0; n<measure.notes.count-1; n++) {
                            OveNote *tmpNote=measure.notes[n];
                            if (tmpNote==note) {
                        ...
                        values_count=[self tappedNoteElems:values note:note nextNote:nextNote art:art below_note:below_note upper_note:upper_note];
                        if (values_count>8) {
                            if (videoMidi) {
                        ...
                        if (art.art_type==Articulation_Major_Trill && nn<sorted_duration_offset.count-1) {
                            ...
                            if (grace_count<5) {
                                ...
                                values_count*=2;
                                tick_range*=2;
                                NSLog(@"many grace:%d",grace_count);
                            }
                        }
                                    ...
                                    if (evt==0x90 && event.vv>0){ //start note
                                        ...
                                        if (found){
                                            if (ee==0 && art.art_type==Articulation_Major_Trill) {
                                                ...
                                                trill_stop_tick=trill_start_tick+ 480/8*values_count;
                                                if (videoMidi) {
                                                    trill_stop_tick+=120;
                                                }
                                                values_count*=2;
...
//                                            for (int next=tt+1; next<midiEvents.count; next++) {
//                                                Event *nextEvent=midiEvents[next];
//                                                if ((nextEvent.evt&0xf0)==kMIDIMessage_NoteOn && nextEvent.vv>0) {
//                                                    if (cur_event_tick+plusTick<nextEvent.tick) {
//                                                        plusTick=nextEvent.tick-cur_event_tick;
//                                                    }
//                                                    break;
//                                                }
//                                            }
...
//                }
    ...
    for (int i=0; i<midiEvents.count; i++) {
        ...
        [debugString appendFormat:@"%d,%d,%x %d %d,",i,event.tick,event.evt+channel,event.nn, event.vv];
        ...
        if (event.userdata) {
            ...
            [debugString appendFormat:@"%@%@,%d(%d),%@,%@,%@,%@,%@,%@\n",
             event.userdata[@"duration"], note_duration<10 ? @"*" : @"",
             measure.number,measure.show_number,event.userdata[@"nn"],event.userdata[@"ii"],
...
- (MidiFile*)parseMidi:(NSData*)midi_data videoMidi:(BOOL)isVideoMidi{
    ...
    if (midiFile) {
        [self checkMidiSequence:midiFile videoMidi:isVideoMidi];
    }
...
//    NSMutableArray *segments=[NSMutableArray new];
    ...
    int last_mm=0,last_nn=0;
    int backTick=0;
    for (int i=0;i<midiEvents.count;i++) {
        Event *e=midiEvents[i];
        if (e.userdata) {
            int mm=[e.userdata[@"mm"] intValue];
            int nn=[e.userdata[@"nn"] intValue];
            if (mm>last_mm || (mm==last_mm && nn>=last_nn)) {
                Event *newEvt=[[Event alloc] init];
                newEvt.tick=e.tick-backTick;
                newEvt.evt=e.tick;
                newEvt.nn=e.nn;
                newEvt.vv=e.vv;
                newEvt.userdata=e.userdata;
                [events addObject:newEvt];
                for (int k=i+1; k<midiEvents.count; k++) {
                    Event *stop=midiEvents[k];
                    if (stop.nn==e.nn) {
                        Event *newEvt=[[Event alloc] init];
                        newEvt.tick=stop.tick-backTick;
                        newEvt.evt=stop.tick;
                        newEvt.nn=stop.nn;
                        newEvt.vv=stop.vv;
                        [events addObject:stop];
                        break;
                    }
                }
                last_mm=mm;
