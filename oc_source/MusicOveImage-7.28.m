...
//            real_screen_size=[[UIScreen mainScreen] currentMode].size;
...
- (CGRect) getBeamRect:(OveBeam*)beam start_x:(float) start_x start_y:(float)start_y measure:(OveMeasure*)measure reload:(BOOL)reload
{
    ...
    if (drawPos.size.width==0 || reload) {
        BeamElem *elem0=[beam.beam_elems objectAtIndex:0];
...
[self.svgXmlContent appendString:[NSString stringWithFormat:@"<text x='%.1f' y='%.1f' font-size='%d' style=\"font-family: 'Aloisen New';\" "fillColor">%@</text>\n", (float)(x), (float)(y), (int)(size*LINE_H*0.1),glyph]]

#define GLYPH_Petrucci_id(x,y,size,glyph, ID) \
[self.svgXmlContent appendString:[NSString stringWithFormat:@"<text id='%@' x='%.1f' y='%.1f' font-size='%d' style=\"font-family: 'Aloisen New';\" "fillColor">%@</text>\n",ID, (float)(x), (float)(y), (int)(size*LINE_H*0.1),glyph]]

#define GLYPH_Petrucci_index(x,y,size,glyph, m,n,e) \
[self.svgXmlContent appendString:[NSString stringWithFormat:@"<text id='%d_%d_%d' x='%.1f' y='%.1f' font-size='%d' style=\"font-family: 'Aloisen New';\" onclick='clickNote(this,%d,%d,%d)'>%@</text>\n",m,n,e, (float)(x), (float)(y), (int)(size*LINE_H*0.1),m,n,e,glyph]]

#define GLYPH_Petrucci_id1(x,y,size,glyph, ID) \
[self.svgXmlContent appendString:[NSString stringWithFormat:@"<rect id='%@' x='%.1f' y='%.1f'  width='%d' height='%d' ></rect>\n", ID, (float)(x), (float)(y), (int)(size*LINE_H*3), (int)(LINE_H*3) ]]; \
[self.svgXmlContent appendString:[NSString stringWithFormat:@"<text x='%.1f' y='%.1f' font-size='%d' style=\"font-family: 'Aloisen New';\">%@</text>\n", (float)(x), (float)(y), (int)(size*LINE_H*0.1),glyph]]

#define GLYPH_Petrucci_rotate(x,y,size,rotate,glyph) \
[self.svgXmlContent appendString:[NSString stringWithFormat:@"<text x='%.1f' y='%.1f' font-size='%d' style=\"font-family: 'Aloisen New';\" rotate='%d'>%@</text>\n", (float)(x), (float)(y), (int)(size*LINE_H*0.1),rotate,glyph]]
...
#define ELEM_FLAG_SHARP @"21"//@"23"
#define ELEM_FLAG_FLAT  @"22"//@"62"
#define ELEM_FLAG_STOP  @"23"//@"6e"
#define ELEM_FLAG_DOUBLE_SHARP @"24"//@"dc"
#define ELEM_FLAG_DOUBLE_FLAT @"25"//@"ba"
#define ELEM_FLAG_SHARP_CAUTION @"26"//@"5b"
#define ELEM_FLAG_FLAT_CAUTION @"27"//@"7b"
#define ELEM_FLAG_STOP_CAUTION @"28"//@"4e"
#define ELEM_FLAG_DOUBLE_SHARP_CAUTION @"29"//@"5d"
#define ELEM_FLAG_DOUBLE_FLAT_CAUTION @"2a"//@"d3"
...
#define ELEM_NOTE_FULL      @"41"//@"77"
#define ELEM_NOTE_2_UP      @"42"//@"68"
#define ELEM_NOTE_4_UP      @"43"//@"71"
#define ELEM_NOTE_8_UP      @"44"//@"65"
#define ELEM_NOTE_16_UP     @"45"//@"78"
...
#define ELEM_NOTE_2   @"7c" //@"fa"
#define ELEM_NOTE_4   @"74" //@"cf"
...
- (void)beginSvgImage:(CGSize)size startMeasure:(int)startMeasure
{
    ...
    NSString *xml_header=[NSString stringWithFormat:@"<?xml version=\"1.0\" standalone=\"yes\"?>\n"
                          ...
                          "<html><head>\n"
                          //"<meta name='viewport' content=\"width=device-width, maximum-scale=2.0, minimum-scale=0.1, user-scalable=1\"/>\n"
                          "<meta name='viewport' content=\"width=device-width,  maximum-scale=2.0, minimum-scale=0.1, user-scalable=no\"/>\n"
                          "</head><body "bgColor">\n"
...
- (NSString *)endSvgImage
{
    ...
    //FEFBEB 浅黄色
    //fefefe 浅灰色
    [self.svgXmlContent appendString:@"<div id='popview' style='border:none; width:280px;position:absolute;visibility:hidden;background:none'>\n\
...
//            unsigned char last_numerator=0, last_denominator=0;
            ...
            for (int nn=0; nn<line.bar_count && line.begin_bar+nn<self.music.measures.count; nn++)
            {
                ...
                meas_pos.start_x=x;
                meas_pos.start_y=start_y-LINE_H*2;
                ...
                for (NSString *key in sorted_duration_offset) {
                    ...
                    for (OveNote *note in notes) {
                        ...
                        NoteElem *note_elem=[note.note_elems objectAtIndex:0];
    ...
    if (measure.denominator>0 && measure.numerator>0)
    {
        for (int nn=0; nn<staff_count; nn++) {
            if (measure.denominator==2 && measure.numerator==2) {
                ...
                if (measure.numerator>10) {
                    tmp=[NSString stringWithFormat:@"3%d3%d",measure.numerator/10,measure.numerator%10];
                }
                ...
                if (measure.denominator>10) {
                    tmp=[NSString stringWithFormat:@"3%d3%d",measure.denominator/10,measure.denominator%10];
                }
                ...
                if (!note_tailed_drawed && !note.hideStem && note.note_type!=Note_Whole && !note.inBeam) {
                    ...
                    if (note.isGrace) {//倚音
                        if (note.note_type>=Note_Eight) {
                            ...
                            if (note.stem_up) {
                                LINE(x+LINE_H*YIYIN_ZOOM+0.5, (y>note_y0)?y:note_y0, x+LINE_H*YIYIN_ZOOM+0.5, y-LINE_H*3.0*YIYIN_ZOOM);
                            }else{
                                LINE(x, (y>note_y0)?y:note_y0, x, y+LINE_H*3.0*YIYIN_ZOOM);
                            }
                        }
...
//                                        art_y+=1.5*LINE_H;
                        ...
                        if (![self drawSvgArt:note_art above:art_placement_above x:art_x y:art_y start_y:staff_start_y]) {
                            ...
                            if (art_type == Articulation_Arpeggio)
                            {
                                ...
                                int tmp_y2=start_y+[self lineToY:elem2.line staff:note.staff+elem2.offsetStaff];
