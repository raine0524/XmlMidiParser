...
线的端点: stroke-linecap属性这个属性定义了线段端点的风格，这个属性可以使用butt(平),square(方),round(圆)三个值.
*/
...
bool VmusImage::drawSvgArt(const std::shared_ptr<NoteArticulation>& art, bool art_placement_above, int x, int y, float start_y)
{
	...
	} else if (art_type == Articulation_Natural_Harmonic) {		//一个圆圈
        if (art_placement_above) {
            GLYPH_Petrucci(x+LINE_H*0.4,y-LINE_H*1.0,GLYPH_FONT_SIZE, 0, "c9");
        } else {
            GLYPH_Petrucci(x+LINE_H*0.4,y+LINE_H*1.0,GLYPH_FONT_SIZE, 0, "c9");
        }
    } else if (art_type==Articulation_Marcato) {		//Marcato 着重/重音 音符上面一个大于号“>”
        if (art_placement_above) {
            ART_MARCATO(x+LINE_H*0, y+LINE_H*0.5);
        } else {
            ART_MARCATO(x+LINE_H*0, y+LINE_H*1);
        }
    } else if (art_type==Articulation_Marcato_Dot) {		//Marcato_Dot 着重断奏 音符上面一个大于号“>”下面加一个点
        if (art_placement_above) {
    ...
    } else if (art_type==Articulation_Heavy_Attack) {		//Heavy_Attack 强音 音符上面一个大于号“>”下面加一个横线
        if (art_placement_above) {
    ...
    } else if (art_type==Articulation_Heavier_Attack) {		//Heavier_Attack:特强音 音符上一个^下面加一个横线
        if (art_placement_above) {
    ...
    } else if (art_type == Articulation_Staccatissimo) {		//staccatissimo_placement 短跳音/顿音 音符上面一个实心的三角形
        if (art_placement_above) {
...
void VmusImage::drawSvgRepeat(const std::shared_ptr<OveMeasure>& measure, float start_x, float start_y)
{
    ...
    if (measure->repeat_type!=Repeat_Null)
    {
        ...
        } else if (measure->repeat_type==Repeat_Coda) {		//返回到ToCada
            tmp_x=start_x+(measure->repeate_symbol_pos.start_offset+measure->repeat_offset.offset_x)*OFFSET_X_UNIT;
		...
		for (int i = 0; i < measure->notes.size(); i++)
		{
            ...
            if (note->isRest) {
				...
				for (int elem_nn = 0; elem_nn < note->note_elems.size(); elem_nn++)
                {
                    ...
                    int more_line_num=0;
                    if (elem_nn == note->note_elems.size()-1 && note_elem->line > 5) {		//在本行上面
						more_line_num = (note_elem->line-6)/2+1;
						more_line_y1 = start_y+STAFF_OFFSET[staff-1]-more_line_num*LINE_H;
                    } else if (elem_nn == 0 && note_elem->line < -5) {		//在本行的下面
						more_line_num = (-note_elem->line-6)/2+1;
...
void VmusImage::drawSvgTempo(OveMeasure* first_measure, float start_x, float start_y)
{
	if (!first_measure->tempos.empty()) {
		...
		times["Larghissimo"]						= 35;		// － 极端地缓慢（40 bpm 或以下）
		times["Lentissimo"]							= 38;		// － 比缓板更慢
		times["Grave"]									= 40;		//壮板     ->最慢    沈重的、严肃的
		times["Largo"]									= 44;		//广板     ->很慢    最缓板（现代）或广板
		times["Lento"]									= 52;		//慢板     ->很慢    缓板（40 - 60 bpm）
		times["Adagio"]								= 56;		//柔板    ->慢速    柔板 ／ 慢板（66 - 76 bpm）
		times["Larghetto"]							= 60;		//小广板    ->稍慢    甚缓板（60 - 66 bpm）
		times["Cantabile"]							= 60;		//如歌
		times["Adagietto"]							= 65;		//颇慢
		times["Andante"]								= 66;		//行板      ->行走速度或人的脉搏速度 行板（76 - 108 bpm）
		times["Andantino"]							= 69;		//小行板  ->行走速度或人的脉搏速度
		times["Moderato"]							= 88;		//中板     ->中速或人的脉搏速度
		times["Moderato Scherzando"]	= 88;		//中板     ->中速或人的脉搏速度 中板（90 - 115 bpm）
		times["Allegro moderato"]			= 98;		//中庸的快板  适度、愉快的急速
		times["Allegretto"]							= 108;		//小快板  ->稍快
		times["Allegro"]								= 120;		//快板      ->快速  快板（120 - 168 bpm）
		times["Allegro con allegrezza"]	= 120;		// 欢乐的快板
		times["Allegro assai"]						= 130;
		times["Allegro vivace"]					= 134;		//活泼的快板快板
		times["Vivace"]									= 140;		//速板       ->很快  活泼（~140 bpm）
		times["Vivo"]									= 152;		//速板         ->很快
		times["Presto"]									= 170;		//急板       ->急速 （168 - 200 bpm）
		times["Allegrissimo"]						= 191;
		times["Vivacissimo"]						= 198;
		times["Prestissimo"]						= 204;		//最极板  ->最快 (约为200 - 208 bpm）
